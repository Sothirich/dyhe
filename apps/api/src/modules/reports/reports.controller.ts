  ...
  async exportExcelPerMerchant(
    ...
    try {
      ...
      const formatDateTime = (isoDate?: string | Date) => {
        ...
        hours = hours % 12 || 12;
        return `${String(d.getDate()).padStart(2,'0')}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getFullYear()).slice(-2)} ${String(hours).padStart(2,'0')}:${minutes} ${ampm}`;
      };
      ...
      // Compute Metrics
      // --------------------------
      const packages = inputPackages;
      const totalCount = packages.length;
      const completed = packages.filter(p => ['DELIVERED', 'RETURNED'].includes(p.status));
      const failed = packages.filter(p => ['FAILED', 'PENDING', 'ON_DELIVERY'].includes(p.status));
      ...
      const totalCOD = sum(packages, 'codAmount');
      ...
      const lastDataRow = firstDataRow + packages.length - 1;

      // --- Totals ---
      const totalPackageRow = pickupSheet.addRow(['','','','','','','Total Package', { formula: `COUNTA(A${firstDataRow}:A${lastDataRow})`, result: totalCount }]);
      const totalAmountRow = pickupSheet.addRow(['','','','','','','Total Amount', { formula: `SUM(H${firstDataRow}:H${lastDataRow})`, result: totalCOD }]);
